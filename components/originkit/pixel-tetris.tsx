...
export default function Tetris(props: TetrisProps) {
  ...
  useEffect(() => {
    ...
    function score(cells: Array<[number, number]>, col: number, row: number) {
      ...
      for (const [c, r] of cells) { const gr = row + r; if (gr >= 0) test[gr * cols + (col + c)] = 1; }
      let lines = 0;
      ...
      let aggHeight = 0, holes = 0, bump = 0, prevTop = -1;
    ...
    function spawn() {
      ...
      let bestCol = 0, bestRow = 0, bestScore = -Infinity;
      for (let turn = 0; turn < 4; turn++) {
        ...
        for (let col = 0; col + width < cols; col++) {
          ...
          if (s > bestScore) { bestScore = s; bestCells = cells; bestCol = col; bestRow = row; }
        }
      }
      if (!bestCells) { grid = new Array(cols * rows).fill(-1); piece = null; return; }
      let startRow = 0, width = 0;
      for (const [c, r] of bestCells) { startRow = Math.max(startRow, r); width = Math.max(width, c); }
      startRow = -1 - startRow;
    ...
    function lock() {
      ...
      for (const [c, r] of piece.cells) {
        ...
        if (gr >= 0 && gr < rows && gc >= 0 && gc < cols) grid[gr * cols + gc] = piece.color;
      }
      piece = null;
      const full: number[] = [];
      for (let r = 0; r < rows; r++) {
    ...
    function draw() {
      ...
      ctx.beginPath();
      for (let r = 0; r < rows; r++) for (let c = 0; c < cols; c++) tilePath(c, r);
      ...
      for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
          ...
          if (color === -1) continue;
          ctx.beginPath(); tilePath(c, r);
          ctx.fillStyle = (flashing.has(r) && lit) ? "rgba(255,255,255,0.95)" : rgba(colorFor(color), 1);
          ctx.fill();
        }
      }
      if (piece) {
        ...
        for (const [c, r] of piece.cells) {
          ...
          if (gr < 0) continue;
          ctx.beginPath(); tilePath(piece.col + c, gr); ctx.fill();
        }
      }
    }
    ...
    function loop(time: number) {
      ...
      if (clearMs > 0) {
        clearMs -= dt;
        if (clearMs <= 0) { clearMs = 0; collapse(); spawn(); }
      ...
      if (piece) {
        ...
        while (dropAcc >= dropEvery && piece) {
          ...
          if (piece.row < piece.targetRow) {
            ...
            piece.col = piece.targetCol; lock();
          }
        }
      } else { spawn(); }
  ...
  return (
    <div style={{ ...style, position: "relative", overflow: "hidden", width: "100%", height: "100%", boxSizing: "border-box" }}>
